...
NS_LOG_COMPONENT_DEFINE("AodvBaselineSimulation");

static const uint16_t AODV_PORT = 9000;

// Same simulated wireless loss idea used in GAP and OLSR baseline.
// AODV itself does NOT use CO2 for routing.
...
// ------------------------------
// CO2 packet header for AODV baseline
// ------------------------------
...
// ------------------------------
// AODV sensor application
// ------------------------------
class AodvSensorApp : public Application
{
public:
  AodvSensorApp()
      : m_nodeId(0),
...
private:
  ...
  uint16_t GenerateCo2Value()
  {
    ...
    // Same CO2 generation logic used in GAP and OLSR baseline
    if (m_nodeId % 5 == 0)
    {
  ...
  void StartApplication() override
  {
    ...
    if (m_isGateway)
    {
      InetSocketAddress local = InetSocketAddress(Ipv4Address::GetAny(), AODV_PORT);
      m_socket->Bind(local);
      m_socket->SetRecvCallback(MakeCallback(&AodvSensorApp::ReceivePacket, this));
    }
    else
    {
      m_socket->Connect(InetSocketAddress(m_gatewayAddress, AODV_PORT));

      Simulator::Schedule(Seconds(1.0 + m_nodeId * 0.2),
                          &AodvSensorApp::GeneratePacket,
                          this);
    }
  }
...
uint32_t AodvSensorApp::totalSent = 0;
uint32_t AodvSensorApp::totalReceived = 0;
uint32_t AodvSensorApp::criticalSent = 0;
uint32_t AodvSensorApp::criticalReceived = 0;

double AodvSensorApp::totalDelay = 0.0;
double AodvSensorApp::criticalDelay = 0.0;
...
int main(int argc, char *argv[])
{
  ...
  // Same indoor node positions as GAP and OLSR
  ...
  {
    Ptr<AodvSensorApp> app = CreateObject<AodvSensorApp>();
  ...
  std::cout << "Total packets sent: " << AodvSensorApp::totalSent << std::endl;
  std::cout << "Total packets received at gateway: " << AodvSensorApp::totalReceived << std::endl;

  if (AodvSensorApp::totalSent > 0)
  {
    double pdr = (double)AodvSensorApp::totalReceived / AodvSensorApp::totalSent * 100.0;
    std::cout << "Packet Delivery Ratio: " << pdr << " %" << std::endl;
  }

  std::cout << "Critical packets sent: " << AodvSensorApp::criticalSent << std::endl;
  std::cout << "Critical packets received: " << AodvSensorApp::criticalReceived << std::endl;

  if (AodvSensorApp::criticalSent > 0)
  {
    double criticalPdr = (double)AodvSensorApp::criticalReceived / AodvSensorApp::criticalSent * 100.0;
    std::cout << "Critical Packet Delivery Ratio: " << criticalPdr << " %" << std::endl;
  }

  if (AodvSensorApp::totalReceived > 0)
  {
    double avgDelay = AodvSensorApp::totalDelay / AodvSensorApp::totalReceived;
    std::cout << "Average End-to-End Delay: " << avgDelay << " s" << std::endl;
  }

  if (AodvSensorApp::criticalReceived > 0)
  {
    double avgCriticalDelay = AodvSensorApp::criticalDelay / AodvSensorApp::criticalReceived;
